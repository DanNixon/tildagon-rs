...
async fn main(_spawner: Spawner) {
    ...
    loop {
        let card_1_detect = card_detect_1.is_low().await;
        let card_2_detect = card_detect_2.is_low().await;

        info!("Card 1 detect: {}", card_1_detect);
        info!("Card 2 detect: {}", card_2_detect);

        if card_1_detect == Ok(true) {
            info!("Card 1 type {}", sdcard_1.get_card_type());
            info!("Card 1 size is {} bytes", sdcard_1.num_bytes().unwrap());
        }

        if card_2_detect == Ok(true) {
